'use client';
import { useState, useEffect } from 'react';
import { FiTerminal, FiType, FiColumns, FiCopy, FiRefreshCw, FiDatabase, FiHome, FiChevronRight, FiDownload, FiCode, FiGrid } from 'react-icons/fi';
import Link from 'next/link';

// Schema markup component
function SchemaMarkup() {
  const schema = {
    "@context": "https://schema.org",
    "@type": "WebApplication",
    "name": "CSV Formatter",
    "url": "https://freedevtools.dev/tools/csv-formatter",
    "description": "Free online CSV formatter and converter. Format, validate, and convert CSV data to JSON and TSV with syntax highlighting.",
    "applicationCategory": "UtilityApplication",
    "operatingSystem": "Web Browser",
    "offers": {
      "@type": "Offer",
      "price": "0",
      "priceCurrency": "USD"
    },
    "featureList": [
      "Format and beautify CSV",
      "Validate CSV syntax",
      "Convert CSV to JSON",
      "Convert CSV to TSV",
      "Custom delimiter support",
      "One-click copy to clipboard",
      "Download formatted files"
    ],
    "provider": {
      "@type": "Organization",
      "name": "Free Developer Tools"
    }
  };

  return (
    <script
      type="application/ld+json"
      dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}
    />
  );
}

// Breadcrumb component
function Breadcrumb() {
  return (
    <nav className="flex items-center text-sm text-gray-500 mb-6" aria-label="Breadcrumb">
      <Link href="/" className="hover:text-indigo-600 flex items-center">
        <FiHome className="w-4 h-4 mr-1" />
        Home
      </Link>
      <FiChevronRight className="w-4 h-4 mx-2" />
      <Link href="/tools" className="hover:text-indigo-600">
        Tools
      </Link>
      <FiChevronRight className="w-4 h-4 mx-2" />
      <span className="text-gray-900 font-medium">CSV Formatter</span>
    </nav>
  );
}

// Main component
export default function CsvFormatterClient() {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [error, setError] = useState('');
  const [viewMode, setViewMode] = useState<'csv' | 'json' | 'tsv'>('csv');
  const [copied, setCopied] = useState(false);
  const [autoFormat, setAutoFormat] = useState(true);
  const [delimiter, setDelimiter] = useState(',');
  const [hasHeaders, setHasHeaders] = useState(true);

  // Auto-format on input change
  useEffect(() => {
    if (!autoFormat || !input.trim()) {
      setOutput('');
      setError('');
      return;
    }

    try {
      if (viewMode === 'csv') {
        const parsed = parseCsv(input);
        setOutput(formatCsvOutput(parsed));
      } else if (viewMode === 'json') {
        const parsed = parseCsv(input);
        setOutput(JSON.stringify(csvToJson(parsed), null, 2));
      } else if (viewMode === 'tsv') {
        const parsed = parseCsv(input);
        setOutput(csvToTsv(parsed));
      }
      setError('');
    } catch (e) {
      setError(`Invalid CSV: ${(e as Error).message}`);
      setOutput('');
    }
  }, [input, viewMode, autoFormat, delimiter, hasHeaders]);

  // Parse CSV with custom delimiter
  const parseCsv = (csvText: string): string[][] => {
    if (!csvText.trim()) throw new Error('Empty input');
    
    const lines = csvText.trim().split('\n');
    const result: string[][] = [];
    
    for (const line of lines) {
      if (!line.trim()) continue;
      
      const row: string[] = [];
      let current = '';
      let inQuotes = false;
      
      for (let i = 0; i < line.length; i++) {
        const char = line[i];
        
        if (char === '"') {
          if (inQuotes && line[i + 1] === '"') {
            current += '"';
            i++; // Skip next quote
          } else {
            inQuotes = !inQuotes;
          }
        } else if (char === delimiter && !inQuotes) {
          row.push(current.trim());
          current = '';
        } else {
          current += char;
        }
      }
      
      row.push(current.trim());
      result.push(row);
    }
    
    return result;
  };

  // Format CSV output with proper alignment
  const formatCsvOutput = (data: string[][]): string => {
    if (data.length === 0) return '';
    
    // Calculate column widths for alignment
    const maxWidths: number[] = [];
    for (const row of data) {
      for (let i = 0; i < row.length; i++) {
        const cellLength = row[i]?.length || 0;
        maxWidths[i] = Math.max(maxWidths[i] || 0, cellLength);
      }
    }
    
    return data.map(row => 
      row.map((cell, i) => {
        const needsQuotes = cell.includes(delimiter) || cell.includes('"') || cell.includes('\n');
        const quotedCell = needsQuotes ? `"${cell.replace(/"/g, '""')}"` : cell;
        return quotedCell;
      }).join(delimiter)
    ).join('\n');
  };

  // Convert CSV to JSON
  const csvToJson = (data: string[][]): any[] => {
    if (data.length === 0) return [];
    
    if (hasHeaders && data.length > 1) {
      const headers = data[0];
      return data.slice(1).map(row => {
        const obj: any = {};
        headers.forEach((header, index) => {
          obj[header] = row[index] || '';
        });
        return obj;
      });
    } else {
      return data.map(row => row);
    }
  };

  // Convert CSV to TSV
  const csvToTsv = (data: string[][]): string => {
    return data.map(row => 
      row.map(cell => cell.replace(/\t/g, '\\t')).join('\t')
    ).join('\n');
  };

  // Format CSV manually
  const formatCsv = () => {
    try {
      const parsed = parseCsv(input);
      setOutput(formatCsvOutput(parsed));
      setViewMode('csv');
      setError('');
    } catch (e) {
      setError(`Invalid CSV: ${(e as Error).message}`);
    }
  };

  // Convert functions
  const convertToJson = () => {
    try {
      const parsed = parseCsv(input);
      setOutput(JSON.stringify(csvToJson(parsed), null, 2));
      setViewMode('json');
      setError('');
    } catch (e) {
      setError(`Invalid CSV for JSON conversion: ${(e as Error).message}`);
    }
  };

  const convertToTsv = () => {
    try {
      const parsed = parseCsv(input);
      setOutput(csvToTsv(parsed));
      setViewMode('tsv');
      setError('');
    } catch (e) {
      setError(`Invalid CSV for TSV conversion: ${(e as Error).message}`);
    }
  };

  // Copy to clipboard
  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(output);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch (error) {
      alert('Failed to copy to clipboard');
    }
  };

  // Download output
  const downloadOutput = () => {
    if (!output) return;
    
    const blob = new Blob([output], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `formatted-data.${viewMode}`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  };

  const resetFields = () => {
    setInput('');
    setOutput('');
    setError('');
  };

  return (
    <>
      <SchemaMarkup />
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <Breadcrumb />
          
          {/* Hero Section */}
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-green-100 rounded-full mb-4">
              <FiColumns className="w-8 h-8 text-green-600" />
            </div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              CSV Formatter & Converter
            </h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Format, validate, and convert CSV data to JSON and TSV. Perfect for data analysis, 
              spreadsheet processing, and data transformation tasks.
            </p>
          </div>

          {/* Main Tool */}
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
            {/* Input Section */}
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Enter CSV data:
              </label>
              <textarea
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder='Paste your CSV data here...'
                className="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 h-48 resize-none font-mono text-sm"
              />
              {error && (
                <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-red-600 text-sm">{error}</p>
                </div>
              )}
            </div>

            {/* Settings */}
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <h3 className="text-sm font-medium text-gray-700 mb-3">CSV Settings:</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Delimiter:</label>
                  <select
                    value={delimiter}
                    onChange={(e) => setDelimiter(e.target.value)}
                    className="w-full p-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-green-500"
                  >
                    <option value=",">Comma (,)</option>
                    <option value=";">Semicolon (;)</option>
                    <option value="\t">Tab (\t)</option>
                    <option value="|">Pipe (|)</option>
                  </select>
                </div>
                
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    id="hasHeaders"
                    checked={hasHeaders}
                    onChange={(e) => setHasHeaders(e.target.checked)}
                    className="mr-2 focus:ring-green-500"
                  />
                  <label htmlFor="hasHeaders" className="text-xs font-medium text-gray-600">
                    First row contains headers
                  </label>
                </div>
                
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    id="autoFormat"
                    checked={autoFormat}
                    onChange={(e) => setAutoFormat(e.target.checked)}
                    className="mr-2 focus:ring-green-500"
                  />
                  <label htmlFor="autoFormat" className="text-xs font-medium text-gray-600">
                    Auto format on change
                  </label>
                </div>
              </div>
            </div>

            {/* Options */}
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-700 mb-3">Output Format:</h3>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <button
                  onClick={() => setViewMode('csv')}
                  className={`p-3 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                    viewMode === 'csv'
                      ? 'bg-green-600 text-white'
                      : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                  }`}
                >
                  <FiDatabase className="w-4 h-4 mx-auto mb-1" />
                  CSV
                </button>
                
                <button
                  onClick={() => setViewMode('json')}
                  className={`p-3 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                    viewMode === 'json'
                      ? 'bg-indigo-600 text-white'
                      : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                  }`}
                >
                  <FiCode className="w-4 h-4 mx-auto mb-1" />
                  JSON
                </button>
                
                <button
                  onClick={() => setViewMode('tsv')}
                  className={`p-3 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                    viewMode === 'tsv'
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                  }`}
                >
                  <FiGrid className="w-4 h-4 mx-auto mb-1" />
                  TSV
                </button>
                
                <button
                  onClick={() => setAutoFormat(!autoFormat)}
                  className={`p-3 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                    autoFormat
                      ? 'bg-purple-600 text-white'
                      : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                  }`}
                >
                  <FiRefreshCw className="w-4 h-4 mx-auto mb-1" />
                  Auto Format
                </button>
                
                <button
                  onClick={resetFields}
                  className="p-3 rounded-lg text-sm font-medium bg-red-100 hover:bg-red-200 text-red-600 cursor-pointer transition-colors"
                >
                  <FiRefreshCw className="w-4 h-4 mx-auto mb-1" />
                  Reset
                </button>
              </div>
            </div>

            {/* Action Buttons */}
            <div className="flex flex-wrap gap-4 mb-6">
              {output && (
                <>
                  <button
                    onClick={copyToClipboard}
                    className="bg-gray-200 px-6 py-3 rounded-lg hover:bg-gray-300 flex items-center gap-2 font-medium cursor-pointer transition-colors"
                  >
                    {copied ? <FiRefreshCw className="text-green-600" /> : <FiCopy />}
                    {copied ? 'Copied!' : 'Copy Result'}
                  </button>
                  
                  <button
                    onClick={downloadOutput}
                    className="bg-gray-200 px-6 py-3 rounded-lg hover:bg-gray-300 flex items-center gap-2 font-medium cursor-pointer transition-colors"
                  >
                    <FiDownload />
                    Download
                  </button>
                </>
              )}
            </div>

            {/* Result */}
            {output && (
              <div className="mt-6">
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium text-gray-700">
                    Formatted {viewMode.toUpperCase()}:
                  </label>
                  <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                    {viewMode.toUpperCase()}
                  </span>
                </div>
                <div className="relative">
                  <pre className="w-full p-4 bg-gray-900 text-gray-100 rounded-lg overflow-auto h-64 text-sm font-mono border">
                    <code>{output}</code>
                  </pre>
                </div>
              </div>
            )}
          </div>

          {/* How to Use Guide */}
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
              <FiDatabase className="text-green-600" />
              How to Use the CSV Formatter
            </h2>
            
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-semibold mb-2">Step-by-Step Guide</h3>
                <ol className="list-decimal list-inside space-y-2 text-gray-700">
                  <li>Paste your CSV data in the input field</li>
                  <li>Configure delimiter and header settings</li>
                  <li>Choose your desired output format (CSV, JSON, or TSV)</li>
                  <li>Enable "Auto Format" for real-time formatting</li>
                  <li>Copy the formatted result or download it as a file</li>
                  <li>Use validation to check for CSV syntax errors</li>
                </ol>
              </div>

              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h4 className="font-semibold text-green-800 mb-2">💡 Pro Tips</h4>
                <ul className="list-disc list-inside space-y-1 text-green-700">
                  <li>Use CSV → JSON conversion for API data processing</li>
                  <li>Convert to TSV for tab-separated data interchange</li>
                  <li>Configure custom delimiters for different CSV formats</li>
                  <li>Enable headers option for better JSON object structure</li>
                  <li>Validate CSV structure before importing into databases</li>
                </ul>
              </div>
            </div>
          </div>

          {/* Examples */}
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Format Examples</h2>
            
            <div className="grid md:grid-cols-3 gap-6">
              <div>
                <h3 className="text-lg font-semibold mb-3 text-green-700">CSV</h3>
                <div className="bg-gray-50 p-3 rounded border">
                  <pre className="text-sm text-gray-800 overflow-x-auto">
{`name,age,city
John Doe,30,New York
Jane Smith,25,Los Angeles`}
                  </pre>
                </div>
              </div>
              
              <div>
                <h3 className="text-lg font-semibold mb-3 text-indigo-700">JSON</h3>
                <div className="bg-gray-50 p-3 rounded border">
                  <pre className="text-sm text-gray-800 overflow-x-auto">
{`[
  {
    "name": "John Doe",
    "age": "30",
    "city": "New York"
  },
  {
    "name": "Jane Smith",
    "age": "25",
    "city": "Los Angeles"
  }
]`}
                  </pre>
                </div>
              </div>
              
              <div>
                <h3 className="text-lg font-semibold mb-3 text-blue-700">TSV</h3>
                <div className="bg-gray-50 p-3 rounded border">
                  <pre className="text-sm text-gray-800 overflow-x-auto">
{`name	age	city
John Doe	30	New York
Jane Smith	25	Los Angeles`}
                  </pre>
                </div>
              </div>
            </div>
          </div>

          {/* Related Tools */}
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Related Data Tools</h2>
            <div className="grid md:grid-cols-3 gap-4">
              <Link href="/tools/json-formatter" className="block p-4 border border-gray-200 rounded-lg hover:border-green-300 hover:shadow-md transition-all">
                <div className="flex items-center gap-3 mb-2">
                  <FiCode className="text-indigo-600" />
                  <h3 className="font-semibold">JSON Formatter</h3>
                </div>
                <p className="text-sm text-gray-600">Format, validate, and convert JSON data</p>
              </Link>
              
              <Link href="/tools/css-formatter" className="block p-4 border border-gray-200 rounded-lg hover:border-indigo-300 hover:shadow-md transition-all">
                <div className="flex items-center gap-3 mb-2">
                  <FiTerminal className="text-indigo-600" />
                  <h3 className="font-semibold">CSS Formatter</h3>
                </div>
                <p className="text-sm text-gray-600">Format, minify, and validate CSS code with syntax highlighting</p>
              </Link>
              
              <Link href="/tools/text-formatter" className="block p-4 border border-gray-200 rounded-lg hover:border-indigo-300 hover:shadow-md transition-all">
                <div className="flex items-center gap-3 mb-2">
                  <FiType className="text-indigo-600" />
                  <h3 className="font-semibold">Text Formatter</h3>
                </div>
                <p className="text-sm text-gray-600">Format and beautify plain text with advanced options</p>
              </Link>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}